...
export class ProductMapper {
    ...
    static commercetoolsCategoryToCategory: (commercetoolsCategory: CommercetoolsCategory, locale: Locale) => Category = (
        ...
        return {
            ...
            subCategories: (commercetoolsCategory as any).subCategories.map((subCategory: CommercetoolsCategory) => ProductMapper.commercetoolsCategoryToCategory(subCategory, locale)),
            path:
    ...
    static facetDefinitionsToFilterFacets(
        ...
        queryFacets.forEach((queryFacet) => {
            ...
            switch (typeLookup[queryFacet.identifier]) {
                case 'money':
                    filterFacets.push(
                        `${queryFacet.identifier}.centAmount:range (${(queryFacet as QueryRangeFacet).min} to ${(queryFacet as QueryRangeFacet).max
                        })`,
                    );
                    break;
                case 'enum':
                ...
                default:
                    if (queryFacet.type === FilterTypes.TERM || queryFacet.type === FilterTypes.BOOLEAN) {
                        ...
                        filterFacets.push(
                            `${queryFacet.identifier}:range (${(queryFacet as QueryRangeFacet).min} to ${(queryFacet as QueryRangeFacet).max
                            })`,
                        );
                    }
                    break;
