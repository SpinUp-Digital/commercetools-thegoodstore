import React, { ChangeEvent } from 'react';

export interface Props extends Omit<React.ComponentProps<'input'>, 'onChange'> {
  onChange?: (event: ChangeEvent<HTMLInputElement>) => void;
}

const Checkbox: React.FC<Props> = ({ className = '', ...props }) => {
  const bgImageClassName = `
    checked:bg-[url('data:image/svg+xml;base64,')]
  `;

  const checkboxClassName = `h-[24px] w-[24px] border-neutral-400 bg-white text-white checked:border-neutral-400 ${bgImageClassName} checked:bg-[length:auto_50%] checked:bg-center checked:bg-no-repeat hover:border-neutral-400 checked:hover:border-neutral-400 focus:border-neutral-400 focus:shadow-none focus:shadow-transparent focus:outline-none focus:ring-transparent checked:focus:border-neutral-400`;

  return <input className={`${className} ${checkboxClassName}`} type="checkbox" {...props} />;
};

export default Checkbox;
