...
export default function FormRadioGroup({ headline, subline, options, onChange, className = '' }: FormRadioGroupProps) {
  return (
    <div className={className}>
      <label className="dark:text-light-100 text-base font-medium text-gray-900">{headline}</label>
        ...
        <div className="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
          {options.map((option, index) => (
            <div key={index} className="flex items-center">
              ...
              />
              <label htmlFor={option.id} className="dark:text-light-100 ml-3 block text-sm font-medium text-gray-700">
                {option.label}
              </label>
            </div>
          ))}
