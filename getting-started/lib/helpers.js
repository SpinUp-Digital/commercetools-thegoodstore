...
export const later = (howLong, value) => new Promise((resolve) => setTimeout(() => resolve(value), howLong));
  ...
  const execute = (fn, tries, args) =>
    Promise.resolve()
      ...
      .catch((error) => {
        ...
        return later(COOL_DOWN_PERIOD).then(() => execute(fn, tries, args));
      });
...
export const execute = createLimiter()(createRetry()(client.execute.bind(client)));
export const getAll =
  (getterFn, service) =>
  (request, statusCallback = (x) => x) => {
    const limit = 100;
...
export const setBy = (getter) => (items) => items.reduce((items, item) => items.set(getter(item), item), new Map());
export const setByKey = setBy((item) => item.key);
export const groupBy = (getter) => (items) =>
  items.reduce((items, item) => items.set(getter(item), (items.get(getter(item)) || []).concat(item)), new Map());

export const readFile = (filePath, encoding = 'utf8') =>
  new Promise((resolve, reject) =>
    fs.readFile(filePath, encoding, (err, fileContent) => (err ? reject(err) : resolve(fileContent))),
  );
...
export const processAll = (getterFn, service, request, processor = (x) => x) => {
  const limit = 100;
  const recur = (result, getterFn, service, goOn, lastId, total) => {
    if (!goOn) {
      return result;
    ...
    return getterFn({
      ...
      return Promise.all(response.body.results.map((item) => processor(item, total || response.body.total))).then(
        (processed) => {
...
const processWithNotify = (message, getterFn, service, request, processor = (x) => x) => {
  ...
  let processed = 0;
  return processAll(getterFn, service, request, (current, total) =>
    Promise.resolve()
...
export const createStandardDelete =
  ({
    ...
    deleteFunction = (item) =>
      execute({
        uri: (typeof service === 'function' ? service() : service).byId(item.id).withVersion(item.version).build(),
        method: 'DELETE',
      }),
  }) =>
  () => {
    ...
    return processWithNotify(
      ...
      deleteFunction,
    ).catch((err) => logAndExit(err, `Failed to delete ${itemName}`));
  };
