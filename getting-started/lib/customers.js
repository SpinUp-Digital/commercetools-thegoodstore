import { customersService, customerGroupService } from './services';
import { logAndExit, execute, createStandardDelete } from './helpers';
const nconf = require('nconf');

export const importCustomers = () =>
  Promise.all(
    require(process.cwd() + '/data/customers.json').map((customer) =>
      execute({
        uri: customersService.build(),
        method: 'POST',
        body: { ...customer, key: customer.id },
      }),
    ),
  )
    .then(() =>
    ...
    .catch((error) => logAndExit(error, 'Failed to add customers'));

export const importCustomerGroups = () =>
  Promise.all(
    require(process.cwd() + '/data/customer-groups.json').map((customerGroup) =>
      execute({
    ...
    .catch((err) => logAndExit(err, 'Failed to import customer groups'));
