...
import { execute, logAndExit, createStandardDelete, removeEmptyStrings } from './helpers';
...
const setCateoryData = (category) => ({ ...category });
const setParent = (categories) => {
  const byId = categories.reduce((result, category) => result.set(category.id, category), new Map());
  return Array.from(byId.values()).map((category) => {
    const categoryCopy = setCateoryData(category);
    categoryCopy.parent = categoryCopy.parentId ? byId.get(categoryCopy.parentId).key : undefined;
...
const groupByParent = (categories) => {
  ...
  return Array.from(recur(categories, new Map([[0, []]]), [undefined], 0).values()).filter(
...
const normalizeCategory = (category) => {
  ...
  return { ...category, key: category.key || key };
};
export const importCategories = () =>
  require('csvtojson')()
    .fromFile(process.cwd() + '/data/categories.csv')
    .then((rawJson) => rawJson.map(normalizeCategory).map(removeEmptyStrings))
    .then((rawJson) => saveRecursive(groupByParent(setParent(rawJson))))
    .then(() =>
      // eslint-disable-next-line no-console
      console.log('\x1b[32m%s\x1b[0m', 'Categories imported'),
    )
    .catch((err) => logAndExit(err, 'Failed to import categories'));
