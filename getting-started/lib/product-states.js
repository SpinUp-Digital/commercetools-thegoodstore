...
export const deleteAllProductStates = () =>
  createStandardDelete({
    ...
    service: () => stateService.where('transitions is not empty and type = "ProductState"'),
    ...
    createStandardDelete({
      itemName: 'states',
      service: () => stateService.where('type = "ProductState"'),
    }),
  );
...
export const importProductStates = () =>
  execute({
    ...
    .catch((err) => logAndExit(err, 'Failed to import product states'));

if (nconf.get('clean')) {
  deleteAllProductStates();
} else if (nconf.get('import')) {
  // eslint-disable-next-line no-console
  console.log('\x1b[32m%s\x1b[0m', 'Importing product item states...');
  importProductStates();
}
