...
export const deleteAllLineItemStates = () =>
  createStandardDelete({
    ...
    service: () => stateService.where('transitions is not empty and type = "LineItemState"'),
    deleteFunction: (result) =>
      execute({
        uri: stateService.byId(result.id).withVersion(result.version).build(),
        method: 'POST',
        body: {
    ...
    createStandardDelete({
      itemName: 'states',
      service: () => stateService.where('builtIn = false and type = "LineItemState"'),
    }),
  );
...
export const importLineItemStates = () =>
  execute({
    ...
    .then((initialId) =>
      Promise.all(
        [
          ...
          ['backorder', 'In replenishment', 'Wird nachbestellt'],
          ['shipped', 'Shipped', 'Versandt'],
          ['canceled', 'Canceled', 'Storniert'],
          ['picking', 'Picking', 'Picking'],
          ['returned', 'Returned', 'Retourniert'],
          ['returnApproved', 'Return approved', 'Retoure akzeptiert'],
          ['returnNotApproved', 'Return not approved', 'Retoure nicht akzeptiert'],
          ['closed', 'Closed', 'Abgeschlossen'],
          ['lost', 'Lost', 'Verloren gegangen'],
          ['lossApproved', 'Loss Approved', 'Verlust bestätigt'],
          ['lossNotApproved', 'Loss not Approved', 'Wieder gefunden'],
        ]
          .map(createState)
          .concat(initialId),
      ),
    ...
    .catch((err) => logAndExit(err, 'Failed to import states'));
